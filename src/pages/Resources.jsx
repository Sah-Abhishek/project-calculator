...
const BillableToggle = ({ isBillable, onChange }) => {
  ...
  useEffect(() => { setChecked(isBillable) }, [isBillable]);
  const handleChange = () => {
  ...
  return (
    <label className="inline-flex items-center cursor-pointer">
      ...
      <span className={`ml-2 text-sm font-medium ${checked ? 'text-accent' : 'text-gray-700'}`}>
...
// --- MODAL COMPONENT (GENERIC FOR ADD/EDIT) ---
const ResourceModal = ({ isOpen, onClose, resource, onSave, projects, subprojects }) => {
  const initialFormState = {
    name: '', email: '', role: '', avatar_url: '',
    assigned_projects: [], assigned_subprojects: [], isBillable: true
  };
  ...
  useEffect(() => {
    if (formData.assigned_projects.length > 0) {
      const related = subprojects.filter(sp => formData.assigned_projects.includes(sp.project_id));
  ...
  const handleAssignmentChange = (type, e) => {
    ...
    const formKey = type === 'project' ? 'assigned_projects' : 'assigned_subprojects';
  ...
  const removeAssignment = (type, idToRemove) => {
    const formKey = type === 'project' ? 'assigned_projects' : 'assigned_subprojects';
    setFormData(prev => ({ ...prev, [formKey]: prev[formKey].filter(id => id !== idToRemove) }));
  };
  ...
  const unassignedProjects = projects.filter(p => !formData.assigned_projects.includes(p._id));
  const unassignedSubProjects = filteredSubprojects.filter(sp => !formData.assigned_subprojects.includes(sp._id));
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl p-8 relative">
        ...
        <div className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2 max-h-24 overflow-y-auto">
                {formData.assigned_projects.map(projectId => (
                  <div key={projectId} className="flex items-center justify-between bg-gray-100 rounded-md px-3 py-1 text-sm">
                    <span>{projects.find(p => p._id === projectId)?.name}</span>
                    <button onClick={() => removeAssignment('project', projectId)} className="text-gray-500 hover:text-red-500">
            ...
            <div>
              ...
              <div className="space-y-2 max-h-24 overflow-y-auto">
                {formData.assigned_subprojects.map(subProjectId => (
                  <div key={subProjectId} className="flex items-center justify-between bg-gray-100 rounded-md px-3 py-1 text-sm">
                    <span>{subprojects.find(sp => sp._id === subProjectId)?.name}</span>
                    <button onClick={() => removeAssignment('subproject', subProjectId)} className="text-gray-500 hover:text-red-500">
                      <XMarkIcon className="w-4 h-4" />
                    </button>
                  </div>
                ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm required font-medium text-gray-700 mb-1">Full Name</label>
              <input type="text" name="name" value={formData.name} onChange={handleStandardChange} required className="w-full border border-gray-300 rounded-lg px-3 py-2" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Role</label>
              <input type="text" name="role" value={formData.role} onChange={handleStandardChange} className="w-full border border-gray-300 rounded-lg px-3 py-2" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="email" name="email" value={formData.email} onChange={handleStandardChange} className="w-full border border-gray-300 rounded-lg px-3 py-2" />
        ...
        <div className="flex justify-end gap-4 mt-8">
          <button onClick={onClose} className="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-semibold">Cancel</button>
          <button
            onClick={handleSubmit}
            disabled={!isFormValid}
            className={`px-6 py-2 rounded-lg font-semibold text-white ${isFormValid ? 'bg-blue-500 hover:bg-blue-700' : 'bg-gray-300 cursor-not-allowed'}`}
          >
  ...
  const DropdownMenu = (
    <div
      ...
      style={{
        ...
        bottom: position === 'top' ? `${window.innerHeight - (coords?.top ?? 0) + 4}px` : '',
...
export default function ResourcesPage() {
  ...
  useEffect(() => {
    if (filters.project) {
      const relatedSubprojects = subprojects.filter(sp => sp.project_id === filters.project);
  ...
  const billableCount = filteredResources.filter(res => res.isBillable).length;
        ...
        <header id="header" className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-500">Assign resources to projects and sub-projects, and toggle their inclusion in billing</p>
            </div>
            <div className="flex items-center space-x-4">
              {/* <button className="bg-gray-100 text-gray-700 px-4 py-2 rounded-xl flex items-center space-x-2 hover:bg-gray-200"><ArrowDownTrayIcon className="w-5 h-5" /><span>Download Template</span></button>
                             <button className="bg-gray-100 text-gray-700 px-4 py-2 rounded-xl flex items-center space-x-2 hover:bg-gray-200"><ArrowUpTrayIcon className="w-5 h-5" /><span>Bulk Upload Excel</span></button> */}
              <button onClick={() => setModalState({ type: 'add', data: null })} className="bg-blue-300 text-gray-900 px-4 py-2 rounded-xl text-lg  flex items-center space-x-2 hover:bg-blue-400"><PlusIcon className="w-7 h-7" /><span>Add Resource</span></button>
              <button className="bg-accent text-white px-4 py-2 rounded-xl flex items-center space-x-2"><ArchiveBoxIcon className="w-5 h-5" /><span>Save All</span></button>
            </div>
          </div>
        </header>
          ...
          <div className="overflow-y-auto flex-1">
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {isLoading ? (
                  <tr><td colSpan="5" className="text-center py-8 text-gray-500">Loading resources...</td></tr>
                ) : (
                  filteredResources.map(res => (
                    <tr key={res._id} className="hover:bg-gray-50">
                      <td className="px-6 py-2 whitespace-nowrap">
                        <div className="flex items-center">
                          <img src={res.avatar_url || 'https://storage.googleapis.com/uxpilot-auth.appspot.com/avatars/avatar-0.jpg'} alt="User" className="w-8 h-8 rounded-full mr-3" />
                          <span className="text-sm font-medium text-gray-900">{res.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4  whitespace-nowrap text-sm text-gray-800">
                        <DropdownList items={getDisplayNames(res.assigned_projects, projects)} label="Projects" />
                      </td>

                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                        <DropdownList items={getDisplayNames(res.assigned_subprojects, subprojects)} label="Sub-Projects" />
                      </td>
                      ...
                      <td className="px-6 py-2 whitespace-nowrap">
                        <div className="flex items-center space-x-3">
                          <button onClick={() => setModalState({ type: 'edit', data: res })} className="text-primary hover:text-blue-700"><PencilSquareIcon className="w-5 h-5" /></button>
                          <button onClick={() => handleDeleteResource(res._id)} className="text-red-500 hover:text-red-700"><TrashIcon className="w-5 h-5" /></button>
                        </div>
                      </td>
                    </tr>
                  ))
          ...
          <div className="bg-gray-50 border-t border-gray-200 px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-6">
                ...
                <div className="flex items-center space-x-2"><span className="text-sm font-medium text-gray-700">Total: <span className="text-gray-900 font-semibold">{filteredResources.length}</span></span></div>
              </div>
